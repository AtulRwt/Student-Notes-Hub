...
import { FaChevronDown, FaBookOpen, FaGraduationCap, FaFileAlt, FaTags } from 'react-icons/fa';
import { useNavigate } from 'react-router-dom';
import type { Course, ResourceType, Tag } from '../../types';
...
type AccordionSection = 'courses' | 'tags' | 'resourceTypes';
...
const CategorySidebar = ({ 
  ...
  const [expandedSections, setExpandedSections] = useState<AccordionSection[]>(['courses']);
  ...
  return (
    <div className="glass rounded-lg p-4 sticky top-4 max-h-[calc(100vh-32px)] overflow-y-auto">
      <h2 className="font-bold text-xl mb-4 gradient-text">Browse Categories</h2>
      
      {/* Accordion: Courses Section */}
      <div className="mb-4">
        <button 
          onClick={() => toggleSection('courses')}
          className="w-full flex items-center justify-between py-2 px-1 border-b border-dark-accent"
        >
          <div className="flex items-center">
            <FaGraduationCap className="mr-2 text-blue-400" />
            <span className="font-semibold">Courses</span>
          </div>
          <FaChevronDown className={`transition-transform ${expandedSections.includes('courses') ? 'rotate-180' : ''}`} />
        </button>
        
        {expandedSections.includes('courses') && (
          <div className="py-2">
            {/* Course category toggle */}
            <div className="flex mb-4 border-b border-dark-accent/30">
              <button
      ...
      <div className="mb-4">
        ...
        >
          <div className="flex items-center">
            <FaTags className="mr-2 text-green-400" />
            <span className="font-semibold">Subject Tags</span>
          </div>
          <FaChevronDown className={`transition-transform ${expandedSections.includes('tags') ? 'rotate-180' : ''}`} />
        </button>
      ...
      {/* Accordion: Resource Types Section */}
      <div className="mb-4">
        <button 
        ...
        >
          <div className="flex items-center">
            ...
            <span className="font-semibold">Resource Types</span>
          </div>
          <FaChevronDown className={`transition-transform ${expandedSections.includes('resourceTypes') ? 'rotate-180' : ''}`} />
        </button>
        
        {expandedSections.includes('resourceTypes') && (
          <div className="flex flex-wrap gap-2 py-3">
            {Object.entries(RESOURCE_TYPES).map(([key, label]) => (
              <button
                key={key}
                ...
                style={{ 
                  backgroundColor: RESOURCE_COLORS[key as ResourceType],
                  color: '#1a1a2e',
                  fontWeight: activeFilters.resourceType === key ? 'bold' : 'normal',
                  opacity: activeFilters.resourceType === key ? 1 : 0.8
                }}
              >
